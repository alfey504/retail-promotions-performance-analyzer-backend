import httpx
from services.api_services.model import FulfillmentHistory, PagedAPIResponse
from services.api_services.api import get_server_uri
from typing import Tuple

def get_fulfillment_by_page(page: int) -> Tuple[int, list[FulfillmentHistory]]:
    try:
        response = httpx.get(get_server_uri(f"/fulfillment-history?page={page}"))
        json_body = response.json()

        parsed_repsonse = PagedAPIResponse[FulfillmentHistory].model_validate(json_body)
        return parsed_repsonse.max_page, parsed_repsonse.data
    except Exception as e:
        raise e
    
def get_all_fulfillment_data() -> list[FulfillmentHistory]:
    try:
